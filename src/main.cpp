...
void game_loop(Game& game1, GLFWwindow* window);

unsigned int SCREEN_WIDTH = 1520;//1140
unsigned int SCREEN_HEIGHT = 855;//641
// globals for the glfw callbacks to access
Game* game = nullptr; // cant just have the game object here cause opengl/glfw stuff has to be init-ed first
InputManager* im = InputManager::get_instance(); // singleton class
...
int main(int argc, char *argv[]) {
    ...
    GLFWwindow* window = glfwCreateWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "Game 1", nullptr, nullptr);
    if (!window) {
    ...
    Game game1(SCREEN_WIDTH, SCREEN_HEIGHT, 5.0f);
    game = &game1;
    game1.set_window(window);
    game1.init();

    game_loop(game1, window);
...
void game_loop(Game& game1, GLFWwindow* window) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        GLEC(glClearColor(0.1f, 0.2f, 0.2f, 1.0f));
